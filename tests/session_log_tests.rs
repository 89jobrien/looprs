...
fn test_writes_valid_jsonl() {
    ...
    let mut logger = SessionLogger::new(dir.path().to_path_buf()).unwrap();
...
fn test_all_event_types_serialize() {
    ...
    logger.log(SessionEvent::UserMessage { content: "q".into(), provider: "ollama".into() }).unwrap();
    logger.log(SessionEvent::Inference { content: "a".into(), provider: "ollama".into() }).unwrap();
