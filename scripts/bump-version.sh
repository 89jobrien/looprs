...
categorize_commits() {
	...
	if [[ ${#feat_commits[@]} -gt 0 ]]; then
		...
		for commit in "${feat_commits[@]}"; do
			...
			local clean_commit
			clean_commit=$(sed -E 's/^feat(\([^)]+\))?:[[:space:]]*//' <<<"$commit")
	...
	if [[ ${#fix_commits[@]} -gt 0 ]]; then
		...
		for commit in "${fix_commits[@]}"; do
			local clean_commit
			clean_commit=$(sed -E 's/^fix(\([^)]+\))?:[[:space:]]*//' <<<"$commit")
			changelog_entries+="- $clean_commit\n"
		done
		changelog_entries+="\n"
	fi
	...
	if [[ ${#docs_commits[@]} -gt 0 ]]; then
		...
		for commit in "${docs_commits[@]}"; do
			local clean_commit
			clean_commit=$(sed -E 's/^docs(\([^)]+\))?:[[:space:]]*//' <<<"$commit")
			changelog_entries+="- $clean_commit\n"
		done
		changelog_entries+="\n"
	fi
	...
	if [[ ${#refactor_commits[@]} -gt 0 || ${#perf_commits[@]} -gt 0 ]]; then
		...
		for commit in "${refactor_commits[@]}"; do
			local clean_commit
			clean_commit=$(sed -E 's/^refactor(\([^)]+\))?:[[:space:]]*//' <<<"$commit")
			changelog_entries+="- $clean_commit\n"
		done
		for commit in "${perf_commits[@]}"; do
			local clean_commit
			clean_commit=$(sed -E 's/^perf(\([^)]+\))?:[[:space:]]*//' <<<"$commit")
...
update_cargo_toml() {
	...
	# Update the workspace version and every internal workspace dependency requirement.
...
update_changelog() {
	...
	local changelog_entries
	changelog_entries=$(categorize_commits <<<"$commits")
...
update_cargo_lock() {
	...
	if [[ "$DRY_RUN" == "true" ]]; then
		log_info "Would run: cargo update --workspace"
		return
	fi
